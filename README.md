...
| **TC-31** | Session PIN Unlock & Entry Opening | 1. With locked entries present and the session locked, click a locked entry or its lock button.<br>2. Enter the valid 4–6 digit PIN.<br>3. Click **"Unlock"**. | The PIN is verified against the stored PBKDF2 hash in constant time (`safeEqual`). On success, the session marks `isUnlocked = true`, all locked entry cards reveal their titles and previews, and the pending locked entry opens in the editor immediately. |
